var ShopLayerMenu = BattleMenu.extend({
    ctor: function (battle) {
        ...
        this.buyGold = function (gem, gold, flag) {
            ...
            BasicPopup.alert("友情提示", content, function () {
                this.falg = true;
            }.bind(this));
        };
        ...
        this.buyGoods = function (goods) {
            ...
            if (PlayerData.getAmountByUnit(price.unit) >= price.value) {
                ...
                if (price.unit === 'gem') {
                    BasicPopup.confirm("友情提示", "当前钻石不足,是否前往充值？", function (result) {
                        if(result){
                            //进入充值页面。
                            RechargePanel.open();
                        }
                    ...
                    BasicPopup.confirm("友情提示", "当前金币不足,点击确定进入点金页面", function (result) {
                        if(result) {
                            this.showMenuLayer("moneyTree_tab");
                        }
                    }.bind(this));
                } else {
                    BasicPopup.confirm("友情提示", "当前该资源不足", function (result) {
